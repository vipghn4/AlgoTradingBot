...
class CurrencyConverter:
    ...
    def convert_trade(self, amount_usd: float, date: pd.Timestamp, is_buy: bool, rate: float) -> float:
        ...
        Terms:
        - Effective FX rate: The actual conversion rate after applying the FX markup fee.
        
        High-level logic:
        1. If the account currency is USD, return the amount unchanged.
        2. If the account currency is not USD, apply the FX conversion rate for the given date.
        3. Apply the FX markup fee based on whether it's a buy or sell transaction.
            3.1. For buy transactions, the effective rate is increased by the FX fee percentage.
            3.2. For sell transactions, the effective rate is decreased by the FX fee percentage.
        4. Return the converted amount in account currency.
        """
        if self.fx_pct == 0.0:
